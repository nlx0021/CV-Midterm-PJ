...
##### FIXME
def loc2bbox_new(src_bbox, loc, scale=1):
    if src_bbox.size()[0] == 0:
        return torch.zeros((0, 4), dtype=loc.dtype)
    ...
    dx          = loc[:, 0::4] * scale
    dy          = loc[:, 1::4] * scale
    dw          = torch.zeros_like(loc[:, 2::4]) * scale
    dh          = torch.zeros_like(loc[:, 3::4]) * scale
    ##### FIXME
...
class DecodeBox():
    ...
    def forward(self, roi_cls_locs, roi_scores, rois, image_shape, input_shape, nms_iou = 0.3, confidence = 0.5):
        ...
        for i in range(bs):
            ...
            roi_cls_loc_new = torch.ones_like(roi_cls_locs[i]) * self.std
            ...
            roi_cls_loc_new = roi_cls_loc_new.view([-1, self.num_classes, 4])
            ...
            ##### FIXME
            cls_bbox_new    = loc2bbox_new(roi.contiguous().view((-1, 4)), roi_cls_loc_new.contiguous().view((-1, 4)))
            cls_bbox_new    = cls_bbox_new.view([-1, (self.num_classes), 4])
            ##### FIXME
            ...
            ##### FIXME
            cls_bbox_new[..., [0, 2]] = (cls_bbox_new[..., [0, 2]]) / input_shape[1]
            cls_bbox_new[..., [1, 3]] = (cls_bbox_new[..., [1, 3]]) / input_shape[0]
            ##### FIXME
            ...
            for c in range(1, self.num_classes):
                ...
                if len(c_confs[c_confs_m]) > 0:
                    ...
                    ##### FIXME
                    boxes_to_process_new = cls_bbox_new[c_confs_m, c]
                    ##### FIXME
                    ...
                    good_boxes  = boxes_to_process_new[keep]
                    print(good_boxes)
                    ##### FIXME
